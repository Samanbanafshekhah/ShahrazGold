...
import { getPendingRegistration, resendRegistrationOtp, verifyOtp } from "@/lib/auth";
...
function OtpPage() {
    ...
    useEffect(() => {
        ...
        setMobile(pending?.mobile ?? null);
        setSeconds(pending?.resendAfter ?? 90);
        if (!pending) nav({ to: "/register", replace: true });
    }, [nav]);
    ...
    async function onResend() {
        ...
        setResending(false);
        if (res.ok) {
            setSeconds(res.resendAfter ?? 90);
            setCode("");
            toast.success("کد جدید ارسال شد.");
        } else {
            toast.error(res.error ?? "ارسال مجدد کد ناموفق بود.");
        }
    }
